...
function getStyles(genres, movieName, theme) {
  return {
    fontWeight:
      movieName.indexOf(genres) === -1
        ...
        >
          {genres.map((genres) => (
            <MenuItem key={genres.id} value={genres.name}>
              <Checkbox checked={movieName.indexOf(genres) > -1} />
...
//To select Artists
export function MultipleSelectArtists() {
    const classes = useStyles();
    const theme = useTheme();
    ...
    return (
      <div className="filterContainer">
        <FormControl className={classes.formControl}>
          ...
          >
            {genres.map((artists) => (
              <MenuItem key={artists.id} value={artists.first_name}>
                <Checkbox checked={artistName.indexOf(artists) > -1} />
                <ListItemText primary={artists.first_name} />
              </MenuItem>
            ))}
          </Select>
        </FormControl>
